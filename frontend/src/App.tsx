...
import { RouteService, type RoutingMode, type RouteResponse } from '@/core/services/RouteService';
...
function App() {
  ...
  // Сервисы
  const routeServiceRef = useRef<RouteService>(new RouteService());
  const isochroneServiceRef = useRef<IsochroneService>(new IsochroneService());
  ...
  const handleMapClick = useCallback((coords: [number, number]) => {
    ...
    setRouteResult(null);
    // Не очищаем endPoint для быстрого пересчёта
  }, []);
  ...
  const handleObjectSelect = useCallback(async (feature: TInfrastructureFeature) => {
    ...
    if (startPoint) {
      await buildRoute(startPoint, coords);
    }
  }, [startPoint]);
  ...
  return (
    <div style={{ 
      ...
      <aside style={{ 
        ...
        {/* Заголовок */}
        <div>
          <h1 style={{ fontSize: '18px', margin: '0 0 8px', fontWeight: '600' }}>
            🗺️ ГИС Таганрог
          </h1>
        ...
        <RoutePanel
          ...
          routeResult={routeResult ? {
            ...
            share_link: routeResult.share_link,
          } : null}
        ...
        <div style={{ 
          ...
          <div style={{ marginBottom: '12px' }}>
            ...
            <select 
              value={isochroneMode} 
              onChange={(e) => setIsochroneMode(e.target.value as 'pedestrian' | 'auto')}
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            <button 
              ...
              disabled={!isochronePoint}
              style={{ 
                flex: 1, 
                padding: '8px', 
                background: isochronePoint ? '#22c55e' : '#94a3b8', 
                color: '#fff', 
                border: 'none', 
                borderRadius: '4px', 
                cursor: isochronePoint ? 'pointer' : 'not-allowed' 
              }}
            >
              🔵 Рассчитать
            </button>
