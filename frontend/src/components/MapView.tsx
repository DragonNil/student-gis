...
import { MAP_CONFIG, LAYER_PRESETS, LAYER_STYLES } from '@/core/config';
...
export default function MapView({
  ...
  // 2. Слои инфраструктуры (🔥 ИСПРАВЛЕНО)
  useEffect(() => {
    ...
    features.forEach((f: any) => {
      // 🔥 ВАЖНО: Сначала ищем тип в properties (для GeoJSON), потом в корне (для плоских объектов)
      const type = f.properties?.type || f.type;

      // Пропускаем, если тип равен "Feature" (это тип самой структуры GeoJSON, а не объекта)
      if (!type || type === 'Feature' || type === 'FeatureCollection') return;

      if (!grouped[type]) grouped[type] = [];
      grouped[type].push(f);
    });

    // Отладка: проверьте в консоли, какие типы нашлись
    console.log('🗺️ Grouped types:', Object.keys(grouped));

    Object.entries(grouped).forEach(([type, arr]) => {
      layerManagerRef.current!.setLayer(type as any, arr);
    });

    Object.keys(LAYER_PRESETS).forEach((type) => {
      const isVisible = activeLayers.includes(type);
      layerManagerRef.current!.toggleLayer(type as any, isVisible);
    });
  ...
  useEffect(() => {
    ...
    const polyline = new (window as any).ymaps.Polyline(coords, {}, {
      strokeColor: colors[currentMode || 'auto'],
      strokeWidth: 4,
      strokeOpacity: 0.9,
    });
  ...
  useEffect(() => {
    ...
    sorted.forEach((feat: any, idx: number) => {
      ...
      const poly = new (window as any).ymaps.Polygon(rings, {
        balloonContent: `Зона: ${feat.properties.time_min} мин`,
      }, {
        fillColor: color,
        strokeColor: color.replace('33', 'cc'),
        strokeWidth: 2,
        opacity: 0.7,
      });
  ...
  useEffect(() => {
    ...
    if (startMarkRef.current) {
      mapInstanceRef.current.geoObjects.remove(startMarkRef.current);
      startMarkRef.current = null;
    }
    if (startPoint) {
      startMarkRef.current = new (window as any).ymaps.Placemark(startPoint, {
        balloonContent: '📍 Старт',
      }, { preset: 'islands#greenCircleDotIcon' });
      mapInstanceRef.current.geoObjects.add(startMarkRef.current);
    }

    if (endMarkRef.current) {
      mapInstanceRef.current.geoObjects.remove(endMarkRef.current);
      endMarkRef.current = null;
    }
    if (endPoint) {
      endMarkRef.current = new (window as any).ymaps.Placemark(endPoint, {
        balloonContent: '🎯 Финиш',
      }, { preset: 'islands#redCircleDotIcon' });
      mapInstanceRef.current.geoObjects.add(endMarkRef.current);
    }
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {isochroneGeoJSON && isochroneGeoJSON.features.length > 0 && (
        <div style={{
          position: 'absolute', bottom: 20, right: 20, background: 'white', padding: '12px 16px',
          borderRadius: 8, boxShadow: '0 4px 12px rgba(0,0,0,0.15)', zIndex: 1000, fontSize: 13,
          fontFamily: 'system-ui, -apple-system, sans-serif', minWidth: 180, pointerEvents: 'none',
        }}>
          ...
          {[...isochroneGeoJSON.features]
            ...
            .map((feat: any) => {
              const idx = [...isochroneGeoJSON.features]
                .sort((a: any, b: any) => a.properties.time_min - b.properties.time_min)
                .findIndex((f: any) => f.properties.time_min === feat.properties.time_min);
              const color = ISO_COLORS[idx % ISO_COLORS.length];
              return (
